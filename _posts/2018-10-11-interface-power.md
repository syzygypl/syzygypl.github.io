...
to think in terms of **contracts** without specifying it's details at the beginning and deferring this moment to 
implementation. One of examples of abstraction is methods defined as **abstract**. Polymorphism gives us ability to 
dynamically use any instance we want in terms of desired contract if it has this contract implemented.
...
contract without any default logic. Let's see how our approach will change if we will swap abstract class to 
interfaces.
...
Here we can see that it was no possible to add `getDescription` to `WebPage` because of inability to add logic to 
...
`Square` should always has equal size so we have modified accessors inherited from `Rectangle`. That what it looks like.
...
rectangle - we breaking the **Liskov Substitution Principle**. The principle defines that objects of a superclass shall 
be replaceable with objects of its subclasses without breaking the application.
```java
public class Main {
...
be able to implement like `setWidth` and `setHeight` it will be easy to spot the problem. Probably in this case 
`Rectangle` should be renamed to `HasArea` and two accessors should be removed. Of course pure contracts can not 
